/*
 ***********************************************************************
 * Copyright (c) 2005-2009, 2014, Freescale Semiconductor, Inc.
...
int App_get_prog_config(AACD_ProgConfig * p)
{
    ...
    {
        printf("coupling channel too large\n");
    }
 ...
 *       buff_len  - Input buffer length
 *
 *   RETURN VALUE
 *       Success : 0
 *         Error : -1
 *       profile not support: -2
*******************************************************************************/
int App_get_adts_header(AACD_Block_Params * params, int buff_len)
{
	...
	for(;;)
	{
        ...
        while (p->syncword != ((1 << LEN_SYNCWORD) - 1))
        {
             ...
             * the ADTS header, then something is wrong, so exit.
             */
            if ((bits_used > (LEN_BYTE * ADTS_FRAME_MAX_SIZE)) || (bits_used>(buff_len*LEN_BYTE)))
                return -1;
	}
	...
	*/
	...
	p->frame_length = App_bs_read_bits(LEN_FRAME_LEN);
      ...
      if(p->frame_length < 0 || p->frame_length > AACD_6CH_FRAME_MAXLEN)
      {
          continue;
      }
      /* if sampling idx and layer is supported, but profile isn't AAC LC, return profile_not_support. */
      if (p->profile != 1)
	{
          return -2;
	}
   ...
   */
    ...
    params->frame_length = p->frame_length - (p->protection_abs ? ADTS_FRAME_HEADER_SIZE : ADTS_FRAME_HEADER_SIZE_CRC);//7;
