...
#define AT803X_HIBERNATE_DISABLE		(1 << 3)
...
static int at803x_probe(struct phy_device *phydev)
{
	...
	if (of_property_read_bool(dev->of_node, "at803x,hibernate-disable"))
		priv->quirks |= AT803X_HIBERNATE_DISABLE;
...
static int at803x_config_init(struct phy_device *phydev)
{
	...
	if (priv->quirks & AT803X_HIBERNATE_DISABLE){
		ret = at803x_debug_reg_mask(phydev, 0xb, 1 << 15, 0);
		if (ret < 0)
			return ret;
	}
