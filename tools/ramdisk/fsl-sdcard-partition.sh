...
slot=""
imgdir="/mnt"
systemimage_file="$imgdir/system.img"
systemimage_raw_file="$imgdir/system_raw.img"
vendor_file="$imgdir/vendor.img"
vendor_raw_file="$imgdir/vendor_raw.img"
partition_file="$imgdir/partition-table.img"
g_sizes=0
append_soc_name=0
support_dtbo=0
flash_m4=0
...
function format_partition
{
    ...
    if [ ${num} -gt 0 ] 2>/dev/null; then
        ...
        mkfs.ext4 -F ${node}p${num} -L$1 >> /dev/null
    fi
}
    ...
    do
        if [ $? -eq 0 ]; then
            if [ $(echo ${1} | grep "system") != "" ] 2>/dev/null; then
                ...
                img_name="$imgdir/boot.img"
            else
                img_name="$imgdir/${1%_*}${soc_name}.img"
            fi
...
function make_partition
{
    if [ ${card_size} -gt 0 ]; then
        partition_file="$imgdir/partition-table-${card_size}GB.img"
    fi
...
function flash_android
{
    ...
    bootloader_file="$imgdir/u-boot${soc_name}.imx"
    ...
    echo "erase_partition: uboot : ${node}boot0"
    echo "flash_partition: ${bootloader_file} ---> ${node}boot0"
    dd if=/dev/zero of=${node}boot0 bs=1k conv=fsync count=4K
    dd if=${bootloader_file} of=${node}boot0 bs=1k seek=${bootloader_offset} conv=fsync
    if [ "${flash_m4}" -eq "1" ] ; then
        m4_image="$imgdir/"${soc_name#*-}"_m4_demo.img"
